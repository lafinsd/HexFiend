...
if {[llength $op_w] > 0} {
  entry "Witness Target opcodes" $wTarget
  entry "op_w len" [llength $op_w]
  section -collapsed "op_w" {
    for {set i 0} {$i < [llength $op_w]}  {incr i} {
      entry [format "op_w %d" $i] [lindex $op_w $i]
    }
  }
}
